...
export default function DecryptedText({
  ...
  // Re-trigger decrypt animation if language/text changes while already in view
  useEffect(() => {
    if (hasAnimated && (animateOn === 'view' || animateOn === 'inViewHover')) {
      triggerDecrypt();
    }
  }, [text, hasAnimated, animateOn, triggerDecrypt]);
    ...
    >
      ...
      <span aria-hidden="true">
        {displayText.split('').map((char, index) => {
          ...
          const currentClass = isRevealedOrDone
            ? `${className} ${isHighlighted ? highlightClassName : ''}`.trim()
            : `${encryptedClassName} ${isHighlighted ? highlightClassName : ''}`.trim();
